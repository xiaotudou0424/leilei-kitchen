...
function generateQRCode(text, container, size = 200) {
  ...
  try {
    ...
    // 备用：使用在线QR Code API生成图片
    const qrSize = Math.min(size, 200);
    const encodedText = encodeURIComponent(text);
    const qrImg = document.createElement('img');
    qrImg.src = `https://api.qrserver.com/v1/create-qr-code/?size=${qrSize}x${qrSize}&data=${encodedText}`;
    qrImg.alt = '点餐二维码';
...
function showQRCode() {
  // 固定使用部署的网站链接
  const baseUrl = 'https://leleixiaozhuzhu.github.io/leilei-kitchen';
